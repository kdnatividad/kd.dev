          ...
          <div className="flex flex-col gap-8">
            {workData.map((job, i) => (
              <div key={i} className="flex gap-4 relative">
                {i !== workData.length - 1 && (
                  <div className="absolute left-5 top-10 bottom-[-32px] w-px bg-gray-700" />
                )}

                <div className="w-10 h-10 rounded-full overflow-hidden shrink-0 border border-gray-700 bg-gray-800 z-10">
                  <img
                    src={job.logo}
                    alt={job.company}
        ...
        >
          <div className="flex flex-col gap-8">
            {educData.map((school, i) => (
              <div key={i} className="flex gap-4 relative">
                {i !== workData.length - 1 && (
                  <div className="absolute left-5 top-10 bottom-[-25px] w-px bg-gray-700" />
                )}
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 px-2">
                    ...
                    <span className="text-gray-400 text-sm">{school.date}</span>
                  </div>
                  <ul className="list-disc ml-4 text-sm px-2">
